...
export async function getRecommendedArticles(currentSlug) {
  ...
  if (otherArticles.length <= 3) return otherArticles;

  // Deterministic pick: the three articles that follow the current one in
  // feed order, wrapping around. Varied per page but stable across builds —
  // a random shuffle here made every build's static output differ.
  const start = currentIndex === -1 ? 0 : currentIndex % otherArticles.length;
  return Array.from({ length: 3 }, (_, i) => otherArticles[(start + i) % otherArticles.length]);
}
