...
/**
 * Convert existing static content to Pages CMS format
 * This script extracts content from React components and creates CMS-compatible JSON files
 */
...
[contentDir, booksDir, timelineDir].forEach(dir => {
  if (!fs.existsSync(dir)) {
    fs.mkdirSync(dir, { recursive: true });
  }
});

// Extract book data from HomePage.js
const homePagePath = path.join(__dirname, '../src/pages/HomePage.js');
const homePageContent = fs.readFileSync(homePagePath, 'utf8');

// Extract the books array using regex
const booksArrayMatch = homePageContent.match(/\{[\s\S]*?\}\.map\(\(book, i\) => \(/);
const booksArrayString = booksArrayMatch ? booksArrayMatch[0].replace(/\.map\(.*$/, '') : '';

if (booksArrayString) {
  // Parse the books array (simplified parsing)
  const bookEntries = [
    {
  ...
  // Create individual book files for CMS
  bookEntries.forEach(book => {
    const fileName = `${book.title.replace(/[^a-zA-Z0-9]/g, '_')}.json`;
    ...
    fs.writeFileSync(filePath, JSON.stringify(cmsBook, null, 2));
...
fs.writeFileSync(
  path.join(contentDir, 'hero.json'),
  JSON.stringify(heroContent, null, 2)
);
console.log('✅ Created: hero.json');

// Create about-bio content
...
fs.writeFileSync(
  path.join(contentDir, 'about-bio.json'),
  JSON.stringify(aboutBioContent, null, 2)
);
...
timelineContent.forEach(yearData => {
  const filePath = path.join(timelineDir, `${yearData.year}.json`);
  fs.writeFileSync(filePath, JSON.stringify(yearData, null, 2));
  console.log(`✅ Created: timeline/${yearData.year}.json`);
});
...
console.log('✅ Created: timeline/index.json');
...
console.log('\n🚀 Next steps:');
console.log('   1. Commit these files to Git');
